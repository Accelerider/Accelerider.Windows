...
namespace Accelerider.Windows.Core.NetWork
{
    ...
    public class Cookies
    {
        ...
        {
            get => JsonConvert.SerializeObject(_cookiesKv);
            set
            {
                try
                {
                ...
                catch
                {
                    ...
                    {
                        if (cookie.Contains("=") && !_cookiesKv.ContainsKey(cookie.Split('=')[0].Replace(" ", string.Empty)))
                        {
        ...
        private static CookieCollection GetAllCookies(CookieContainer cookieContainer)
        {
            ...
            foreach (var item in ((IDictionary)cookieContainer.GetType().GetRuntimeFields().First(v => v.Name == "m_domainTable").GetValue(cookieContainer)).Values)
            {
